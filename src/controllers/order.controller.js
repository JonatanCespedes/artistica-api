...
module.exports = {
    ...
    try {
      ...
      if (item.quantity > 1) {
        ...
        return updateItemResult
          ...
          : res.status(400).json("No se pudo actualizar el item");
      }
      if (item.quantity <= 1) {
        ...
        if (!orderHaveMoreItems) {
          ...
          return res
            .status(200)
            .json("Item eliminado y orden eliminada correctamente");
        }
        return res.status(200).json("Item eliminado de la orden correctamente");
      }
    } catch (error) {
      console.error(error);
      res.status(500).json({ error });
  ...
  removeAllFromOrder: async (req, res) => {
    try {
      ...
      const orderHaveMoreOfOneItems = itemsOrder.length > 1;

      if (orderHaveMoreOfOneItems) {
        // Si la orden tiene mas de 1 item, elimino el item
        const itemDeleteResult = await deleteOrderItem(itemId);

        itemDeleteResult
          ? res.status(200).json("Item eliminado correctamente")
          : res.status(400).json("El item no se pudo eliminar");
      } else {
        // Si la orden tiene 1 item, elimino el item, elimino la orden
        const itemDeleteResult = await deleteOrderItem(itemId);
        const orderDeleteResult = await deleteOrder(item.orderId);

        itemDeleteResult && orderDeleteResult
          ? res
              .status(200)
              .json("Item eliminado y orden eliminada correctamente")
          : res.status(400).json("El item o la orden no se pudo eliminar");
      }
    } catch (error) {
      console.error(error);
      res.status(500).json({ error });
